function Keyboard(props) {
  ...
  return (
    <div className='max-w-sm mx-auto mt-4 space-y-1'>
        {keys.map((row, index) =>
          <div className="flex items-center justify-center space-x-1" key={`row-${index}`}>
            {row.map((button, index) =>
                <button
                  type="button"
                  className="rounded-lg bg-gray-50 border py-1 px-2 uppercase key"
                  key={button}
                ...
                >
                  {button === 'Backspace' ? (
                      <svg width="20" height="20" viewBox="0 0 24 24" className="flex items-center">
